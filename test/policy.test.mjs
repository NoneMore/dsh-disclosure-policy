...
test('the progress primitive and reminders have a tight fixed-context budget', () => {
  ...
  assert.ok(DISCLOSURE_POLICY_TEXT.length < 180)
  assert.ok(DISCLOSURE_REMINDER_TEXT.length < 130)
  assert.ok(DISCLOSURE_REPEAT_TEXT.length < 100)
  assert.match(DISCLOSURE_TOOL_DESCRIPTION, /Proactively/)
  assert.match(DISCLOSURE_TOOL_DESCRIPTION, /findings/)
  assert.match(DISCLOSURE_TOOL_DESCRIPTION, /plan shifts/)
  ...
  assert.match(DISCLOSURE_POLICY_TEXT, /Proactively call `disclose_progress`/)
  assert.match(DISCLOSURE_POLICY_TEXT, /key findings/)
  assert.match(DISCLOSURE_POLICY_TEXT, /phase\/plan changes/)
  assert.match(DISCLOSURE_POLICY_TEXT, /verification results/)
  assert.match(DISCLOSURE_POLICY_TEXT, /blockers/)
  assert.match(DISCLOSURE_POLICY_TEXT, /sustained work/)
  assert.match(DISCLOSURE_POLICY_TEXT, /do not wait for a reminder/)
  assert.equal(DISCLOSURE_POLICY_TEXT.split(/[.!?](?:\s|$)/).filter(Boolean).length, 1)
