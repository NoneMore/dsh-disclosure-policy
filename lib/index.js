...
export function apply(ctx, rawConfig = {}) {
    ...
    ctx.on('session/event', (session, event) => {
        switch (event.type) {
            ...
            case 'assistant/message': {
                ...
                interval.progressAttemptStep = event.data.message.content.some(block => block.type === 'tool-call' && block.name === DISCLOSURE_TOOL_NAME)
                    ? event.data.step
                    : null;
                return;
            }
    ...
    ctx.on('tools/post-execute', async (exec, _result, next) => {
        ...
        const observe = () => {
            ...
            if (interval.currentStep !== null && interval.checkpointStep === interval.currentStep)
                return null;
