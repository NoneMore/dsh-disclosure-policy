...
export function apply(ctx, rawConfig = {}) {
    ...
    ctx.on('session/event', (session, event) => {
        switch (event.type) {
            ...
            case 'assistant/message': {
                ...
                interval.pendingDisclosureStep = event.data.message.content.some(block => block.type === 'tool-call' && block.name === DISCLOSURE_TOOL_NAME) ? event.data.step : null;
                return;
            }
            case 'turn/end':
    ...
    ctx.on('tools/post-execute', async (exec, _result, next) => {
        ...
        if (interval.step !== null && interval.pendingDisclosureStep === interval.step)
            return downstream;
        ...
        if (interval.step !== null && interval.remindedStep === interval.step)
            return downstream;
