...
import { classifyToolActivity, countCompletedCall, createActivity, createSilence, DEFAULT_CONFIG, DISCLOSURE_PLUGIN_NAME, DISCLOSURE_POLICY_ORDER, DISCLOSURE_POLICY_SECTION_NAME, DISCLOSURE_POLICY_TEXT, DISCLOSURE_TOOL_DESCRIPTION, DISCLOSURE_TOOL_NAME, inspectionActivityFact, markReminderDelivered, recordActivity, reminderTextFor, resetSilence, resolveConfig, withReminder, } from './policy.js';
 ...
 * child agents.
 */
function installForAgent(agentCtx, agent, config) {
    let interval;
...
export function apply(ctx, rawConfig = {}) {
    ...
    const install = (agent) => {
        if (installed.has(agent) || !isEligibleAgent(ctx, agent))
            return;
