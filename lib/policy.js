 ...
 */
export const DISCLOSURE_TOOL_DESCRIPTION = 'Proactively update after findings, plan shifts, checks, blockers, or long work: done, next, approach; batch with work.';
/**
...
export const DISCLOSURE_POLICY_TEXT = 'Proactively call `disclose_progress` at long-task milestones—key findings, phase/plan changes, verification results, blockers, or sustained work; do not wait for a reminder.';
...
export function resolveConfig(input = {}) {
    const { reminderAfterCalls = DEFAULT_CONFIG.reminderAfterCalls, maxReminderIntervalCalls = DEFAULT_CONFIG.maxReminderIntervalCalls, activityWindowSize = DEFAULT_CONFIG.activityWindowSize, inspectionHintMinInspections = DEFAULT_CONFIG.inspectionHintMinInspections, } = input;
 ...
 */
function reminderIntervalFor(index, reminderAfterCalls, maxReminderIntervalCalls) {
...
export function countCompletedCall(state, reminderAfterCalls, maxReminderIntervalCalls, options = {}) {
    if (options.nested === true)
        return null;
    state.calls += 1;
    if (reminderAfterCalls <= 0)
        return null;
 ...
 */
export const DISCLOSURE_REMINDER_TEXT = '[disclosure] Call disclose_progress now with a brief checkpoint; if work remains, batch it with the next work tool(s).';
