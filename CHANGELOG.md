...
- Limit the disclosure surface to exact-native live runtime roots. PTC/both Agents and runtime-owned child/subagent Agents receive no `disclose_progress` declaration, cadence state, or reminder listener. Eligibility is sampled from `AgentRegistry.roots()` plus the Agent's effective ToolRuntime view after setup. See ADR-0008.
