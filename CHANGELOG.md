...
- Fix checkpoint-step accounting so top-level ordinary siblings are carried into the fresh disclosure interval instead of disappearing when batched with `disclose_progress`; later same-step siblings, including a PTC enclosing `run_code`, keep advancing cadence while reminder delivery remains fenced to a later model step.
